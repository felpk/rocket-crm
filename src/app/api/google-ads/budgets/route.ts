...
const log = createLogger("google-ads/budgets");

export async function GET(req: Request) {
  log.info("GET /api/google-ads/budgets");
  ...
  try {
    if (process.env.GOOGLE_ADS_DEMO === "true") {
      ...
      const data = await getBudgetUtilization("demo", "demo");
    ...
    try {
      const data = await getBudgetUtilization(
        token.customerId,
        token.accessToken,
        dateRange,
      ...
      if (errStr.includes("401") || errStr.includes("UNAUTHENTICATED")) {
        const result = await refreshAndRetry(userId, (newToken) =>
          getBudgetUtilization(token.customerId, newToken, dateRange, token.loginCustomerId)
        );
        if (result) {
          await prisma.googleAdsConnection.update({
            where: { userId },
