...
const log = createLogger("google-ads/ads");

export async function GET(req: Request) {
  log.info("GET /api/google-ads/ads");
  ...
  try {
    if (process.env.GOOGLE_ADS_DEMO === "true") {
      ...
      const data = await getAdMetrics("demo", "demo");
    ...
    try {
      ...
      if (errStr.includes("401") || errStr.includes("UNAUTHENTICATED")) {
        const result = await refreshAndRetry(userId, (newToken) =>
          getAdMetrics(token.customerId, newToken, dateRange, token.loginCustomerId)
        );
        if (result) {
          await prisma.googleAdsConnection.update({
            where: { userId },
