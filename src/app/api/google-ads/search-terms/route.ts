...
const log = createLogger("google-ads/search-terms");

export async function GET(req: Request) {
  log.info("GET /api/google-ads/search-terms");
  ...
  try {
    if (process.env.GOOGLE_ADS_DEMO === "true") {
      ...
      const data = await getSearchTerms("demo", "demo");
    ...
    try {
      ...
      if (errStr.includes("401") || errStr.includes("UNAUTHENTICATED")) {
        const result = await refreshAndRetry(userId, (newToken) =>
          getSearchTerms(token.customerId, newToken, dateRange, token.loginCustomerId)
        );
        if (result) {
          await prisma.googleAdsConnection.update({
            where: { userId },
