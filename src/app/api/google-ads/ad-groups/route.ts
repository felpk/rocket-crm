...
const log = createLogger("google-ads/ad-groups");

export async function GET(req: Request) {
  log.info("GET /api/google-ads/ad-groups");
  ...
  try {
    if (process.env.GOOGLE_ADS_DEMO === "true") {
      ...
      const data = await getAdGroupMetrics("demo", "demo");
    ...
    try {
      const data = await getAdGroupMetrics(
        token.customerId,
        token.accessToken,
        dateRange,
      ...
      if (errStr.includes("401") || errStr.includes("UNAUTHENTICATED")) {
        const result = await refreshAndRetry(userId, (newToken) =>
          getAdGroupMetrics(token.customerId, newToken, dateRange, token.loginCustomerId)
        );
        if (result) {
          await prisma.googleAdsConnection.update({
            where: { userId },
